...
def main():
    ...
    # Merge: config provides defaults, CLI args override when explicitly provided.
    # sys.argv check detects which args the user actually passed on the command line.
    def _resolve(config_key, cli_val, cli_flag):
        """Return CLI value if explicitly passed, else config value, else CLI default."""
        if any(a.startswith(cli_flag) for a in sys.argv[1:]):
            return cli_val
        return config.get(config_key, cli_val)

    trainer_kwargs = {
        "teacher_weights": _resolve("teacher_weights", args.teacher, "--teacher"),
        "student_weights": _resolve("student_weights", args.student, "--student"),
        "data": _resolve("data", args.data, "--data"),
        "imgsz": _resolve("imgsz", args.imgsz, "--imgsz"),
        "epochs": _resolve("epochs", args.epochs, "--epochs"),
        "batch_size": _resolve("batch_size", args.batch, "--batch"),
        "alpha": _resolve("alpha", args.alpha, "--alpha"),
        "beta": _resolve("beta", args.beta, "--beta"),
        "temperature": _resolve("temperature", args.temperature, "--temperature"),
        "lr0": _resolve("lr0", args.lr0, "--lr0"),
        "momentum": _resolve("momentum", args.momentum, "--momentum"),
        "weight_decay": _resolve("weight_decay", args.weight_decay, "--weight-decay"),
        "warmup_epochs": _resolve("warmup_epochs", args.warmup_epochs, "--warmup-epochs"),
        "device": _resolve("device", args.device, "--device"),
        "project": _resolve("project", args.project, "--project"),
        "name": _resolve("name", args.name, "--name"),
        "save_period": _resolve("save_period", args.save_period, "--save-period"),
        "workers": _resolve("workers", args.workers, "--workers"),
        "feature_loss_type": _resolve("feature_loss_type", args.feature_loss, "--feature-loss"),
        "normalize_features": _resolve("normalize_features", args.normalize_features, "--normalize-features"),
        "teacher_half": _resolve("teacher_half", args.teacher_half, "--teacher-half"),
        "amp": _resolve("amp", args.amp, "--amp"),
        "nbs": _resolve("nbs", args.nbs, "--nbs"),
        "cache": _resolve("cache", args.cache, "--cache"),
    }
